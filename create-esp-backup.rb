...
def create_snapshot
	createsnapshot = `/usr/bin/rsync -avr --exclude-from '/opt/apps/espbackup/etc/rsync/exclude.txt' /opt/fast/ /opt/apps/espbackup/tmp/`
	puts createsnapshot
end
...
# Clean fast archive directory.
Dir.chdir('/opt/apps/espbackup/achive')
system "/bin/rm -rf /opt/apps/espbackup/archive/*"
puts "Cleaned fast archive directory."

# Clean adminserver db backup directory.
Dir.chdir('/opt/fast/')
system "/bin/rm -rf /opt/fast/adminserverbackup/*"
...
# Create archive file.
system "/bin/tar -C /opt/fast -cf /opt/apps/espbackup/arcive/fast . "
